function ErrStatus = rstd_init(RSTD_DLL_Path, u, cip, fixport)
    ...
    adcstartTime = 6; %us
    rampEndTime=60; %us
    freqSlope=65.998; %MHz/us
    txStartTime=0; %us
    numAdcSamples=200;
    SampleRate=5000; %ksps
    frameCount=512; %receive one minute %use 0 for endless recording
    loopCount=16;
    periodicity=50;
    ...
    Bss_file = 'C:\\ti\\mmwave_studio_02_01_01_00\\rf_eval_firmware\\radarss\\xwr18xx_radarss.bin';
    ...
    pause(2);
    Mss_file = 'C:\\ti\\mmwave_studio_02_01_01_00\\rf_eval_firmware\\masterss\\xwr18xx_masterss.bin';
    ...
    pause(1);
    Lua_String = 'ar1.LVDSLaneConfig(0, 1, 1, 0, 0, 1, 0, 0)';
    ErrStatus = RtttNetClientAPI.RtttNetClient.SendCommand(Lua_String);
    if (ErrStatus ~= 30000)
    ...
    Lua_String = strcat('ar1.ProfileConfig(0, 77,', string(idleTime), ',', string(adcStartTime),',',string(rampEndTime),', 0, 0, 0, 0, 0, 0,', string(freqSlope),',', string(txStartTime),',', string(numAdcSamples),',', string(SampleRate),',',' 0, 0, 30)');
    ErrStatus = RtttNetClientAPI.RtttNetClient.SendCommand(Lua_String);
    if (ErrStatus ~= 30000)
    ...
    Lua_String = strcat('ar1.FrameConfig(0, 2,', string(frameCount),',', string(loopCount),',', string(periodicity),', 0, 0, 1)');
    ErrStatus = RtttNetClientAPI.RtttNetClient.SendCommand(Lua_String);
    if (ErrStatus ~= 30000)
    ...
    %added for path decision
    %date=datetime('today');
    %Lua_String = strcat('ar1.CaptureCardConfig_StartRecord("C:\\Users\\mwendelmuth\\Documents\\RecordedData\\',string(date),'_Test1.bin", 1)');
    %ErrStatus = RtttNetClientAPI.RtttNetClient.SendCommand(Lua_String);
    %if (ErrStatus ~= 30000)
        %disp('XX CCCP Fail XX');
        %writeline(u, "XX CCCP Fail XX", cip, fixport);
